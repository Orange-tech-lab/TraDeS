...
class Detector(object):
  ...
  def run(self, image_or_path_or_tensor, meta={}, img_id=None):
    ...
    if self.opt.dataset == 'mot':
      results = self.filter_box(results, meta['height'], meta['width'])
    ...
    ret = {'results': results, 'tot': tot_time, 'load': load_time,
            ...
            'display': display_time, 'our_time': our_time}
  ...
  def post_process(self, dets, meta, scale=1, output=None):
    dets = generic_post_process(
      ...
      [meta['calib']], meta['height'], meta['width'], output=output)
  ...
  def debug(self, debugger, images, dets, output, scale=1, 
    ...
    pred = debugger.gen_colormap(output['hm'][0].detach().cpu().numpy(), 'hm')
    debugger.add_blend_img(img, pred, 'hm')
    if 'hm_hp' in output:
    ...
    if pre_images is not None:
      ...
      if pre_hms is not None:
        pre_hm = debugger.gen_colormap(
          pre_hms[0].detach().cpu().numpy(), 'pre_hm')
  ...
  def filter_box(self, results, img_h, img_w):
    '''
    For MOT dataset, the object with parts outside image is not annotated. We therefore filter out those box that has a
    part outside image. This is optional and does not affect much on results (Only +-0.2 MOTA found in our experiments)
    '''
